...
public class DqTexture {
    ...
    private boolean mIsMinus = true;
    private int mVerticalTranslate = 0;
    private int mHorizontalTranslate = 0;

    public DqTexture(Context context) {
        ...
        // ドロイドくんテクスチャの作成
        final Bitmap bitmap = BitmapFactory.decodeResource(context.getResources(), R.drawable.dq);
    ...
    public void draw(float[] viewProjectionMatrix, float[] worldMatrix) {
        ...
        {
            float horizontalDivider = 5f / 6f;
            float verticalDivider = 2f / 6f;

            float horizontalStart = horizontalDivider - (1f / 6f);
            float horizontalEnd = horizontalDivider;

            float verticalStart = verticalDivider - (1f / 6f);
            ...
            // 一辺100の正方形
            float length = 500f;
            float left = length / 2f * -1 + mHorizontalTranslate * 2;
            float right = length / 2f + mHorizontalTranslate * 2;
            float top = length / 2f * -1 + mVerticalTranslate * 2;
            float bottom = length / 2f + mVerticalTranslate * 2;
...

        {
            ...
            float horizontalDivider = 2f / 6f;
            float verticalDivider = 6f / 6f;
            ...
            // 一辺100の正方形
            float length = 500f;
            float left = length / 2f * -1 - 500;
            float right = length / 2f - 500;
            float top = length / 2f * -1 + 500 + mVerticalTranslate;
            float bottom = length / 2f  + 500 + mVerticalTranslate;
